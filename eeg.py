...
def calculate_psd(sensor_values, sampling_rate):
    """
    Calculate Power Spectral Density (PSD) from the sensor values.
    Params:
        sensor_values (array-like): Time-series sensor values in microvolts.
        sampling_rate (float): The sampling rate of the signal (in Hz).
    Returns:
        freqs (array): Array of frequencies.
        psd (array): Power Spectral Density values for corresponding frequencies.
    """
    # Step 1: Apply FFT to convert the time-domain signal to the frequency domain
    n_samples = len(sensor_values)
    fft_values = np.fft.fft(sensor_values)
    freqs = np.fft.fftfreq(n_samples, 1 / sampling_rate)

    # Step 2: Compute the power spectrum (magnitude squared)
    power_spectrum = np.abs(fft_values) ** 2

    # Step 3: Normalize to compute Power Spectral Density (PSD)
    psd = power_spectrum / (n_samples * sampling_rate)

    # Keep only positive frequencies
    pos_freqs = freqs[:n_samples // 2]
    pos_psd = psd[:n_samples // 2]

    return pos_freqs, pos_psd


def calculate_band_power_ratios(freq_bands):
    """
    Calculate the ratios between different frequency bands.
    Params:
        freq_bands (dict): A dictionary with power values for each frequency band.
    Returns:
        dict: A dictionary containing the calculated band power ratios.
    """
    # Calculate ratios
...
def find_peak_frequency(freqs, power_spectrum):
    """
    Find the peak frequency (frequency with maximum power).
    Params:
        freqs (array): Array of frequencies.
        power_spectrum (array): Power values corresponding to each frequency.
    Returns:
        float: The frequency where the power is maximal.
    """
...
def apply_fft_with_features(data, sampling_rate):
    """
    Group the data by trial and sensor, apply FFT, and calculate additional frequency-domain features:
    Power Spectral Density (PSD), Band Power Ratios, and Peak Frequency.
    Params:
        data (pd.DataFrame): The dataset containing EEG trials and sensor readings.
        sampling_rate (float): The sampling rate of the EEG signal (in Hz).
    Returns:
        pd.DataFrame: A DataFrame containing frequency-domain features for each sensor.
    """
    ...
    for name, group in grouped:
        ...
        for window in windows:
            if len(window) == 256:
                ...
                # Apply the band-pass filter to clean the signal (0.5 to 30 Hz)
                filtered_sensor_values = bandpass_filter(sensor_values, 0.5, 30, sampling_rate)

                # Apply FFT and calculate Power Spectral Density (PSD)
                freqs, psd = calculate_psd(filtered_sensor_values, sampling_rate)

                # Categorize frequency bands and calculate power in each band
                freq_bands = categorize_frequency_bands(filtered_sensor_values, sampling_rate)

                # Calculate band power ratios
                band_power_ratios = calculate_band_power_ratios(freq_bands)

                # Find the peak frequency (frequency with maximum power)
                peak_freq = find_peak_frequency(freqs, psd)

                # Collect features
                feature_row = {
                    'trial_number': name[0],
                    'sensor_position': name[1],
                    'peak_frequency': peak_freq,
                    'theta_alpha_ratio': band_power_ratios['theta_alpha_ratio'],
                    'delta_beta_ratio': band_power_ratios['delta_beta_ratio']
                }
                feature_row.update(freq_bands)
                freq_domain_features.append(feature_row)
            else:
                print(f"Skipping window for trial {name[0]} sensor {name[1]} due to insufficient data points.")
